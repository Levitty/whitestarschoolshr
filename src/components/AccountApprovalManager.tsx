...
const AccountApprovalManager = () => {
  ...
  const updateUserStatus = async (userId: string, status: UserStatus, isActive: boolean) => {
    try {
      // Get the user profile data first
      ...
      // If approving, create an employee_profiles entry
      if (status === 'active' && isActive) {
        // Get the next employee number
        const { data: empNumData, error: empNumError } = await supabase
          .rpc('get_next_employee_number');

        if (empNumError) {
          console.error('Error getting employee number:', empNumError);
          // Continue without failing - employee number will be generated by trigger
        }

        const employeeNumber = empNumData || `EMP${Date.now()}`;
        ...
        if (!existingEmployee) {
          ...
          if (empError) {
            ...
            toast({
              title: "Warning",
              description: "User approved but employee profile creation failed. Please create manually.",
              variant: "destructive"
            });
          } else {
            toast({
              title: "Success",
              description: `User approved and employee profile created with number ${employeeNumber}.`,
            });
            fetchPendingUsers();
            return;
          }
